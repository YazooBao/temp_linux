...
#define LISEN_MAX   20      //服务器最大接收连接数
#define FD_MAX      10      //FD_SET 支持描述数量
/*macro----------------------------------------------------------------------------*/
/*variables------------------------------------------------------------------------*/
/*function prototype---------------------------------------------------------------*/
/*variables------------------------------------------------------------------------*/
 ...
 * \return socket fd
 */
int on_tcp_server(void)
{
    int sockfd;
    int err;
    int i;
    int connfd;
    int fd_all[FD_MAX];    //保存所有描述符，用于select调用后判断哪个可读
    ...
    for(;;){
        ...
        for(i = 1; i < maxfd; i++){
            if(FD_ISSET(fd_all[i], &fd_select)){
                ...
                if(num > 0){
                    //收到 客户端数据并打印
                    log_d("receive buf from client fd_all[%d] is: %s\n", i, buf);
                    //TODO:处理客户端数据
                    on_process(fd_all[i], buf);
                    //回复客户端
...
int creat_tcp_server(void)
{
    ...
    elog_i("info", "tcp server thread success!\n");
    return err;
}
